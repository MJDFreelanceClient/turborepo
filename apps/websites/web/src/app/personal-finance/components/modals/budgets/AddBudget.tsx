...
export const AddBudget = () => {
    ...
    modal.listen({current:"budget", op:"add"}, () => setOpen(true))

    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger className={`p-4  bg-gray-900 text-white rounded-[12px]`}>+ Add New Budget</DialogTrigger>
                <DialogContent className={`bg-white ${colors.setup} ${text.setup} flex flex-col-gap-5 p-5`}>
                    <DialogHeader className={`text-left flex flex-col gap-5`}>
                        <DialogTitle className={`text-preset-1 text-gray-900 font-bold`}>Add New Budget</DialogTitle>
