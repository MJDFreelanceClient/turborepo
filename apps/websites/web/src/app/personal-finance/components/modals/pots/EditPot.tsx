...
import {getPots, Pot} from "@/lib/dynamo";
import {useQuery} from "@tanstack/react-query";
import {useForm} from "@tanstack/react-form";

export const EditPot = () => {
    const [potId, setPotId] = useState<string|undefined>(); // This should be controlled by your modal state management
    modal.listen({current:"pot", op:"edit"}, (data) => setPotId(data))
    ...
    const form = useForm({
            defaultValues: {
                id: potId ?? '',
                name: data?.find(d=>d.id===potId)?.name ?? '',
                target: data?.find(d=>d.id===potId)?.target ?? 0,
                theme: data?.find(d=>d.id===potId)?.theme ?? '',
            }
        });
    ...
    return (
        <Dialog open={!!potId} onOpenChange={()=>setPotId(undefined)}>
            <DialogContent className={`bg-white ${colors.setup} ${text.setup} flex flex-col-gap-5 p-5`}>
                    <DialogHeader className={`text-left flex flex-col gap-5`}>
                        <DialogTitle className={`text-preset-1 text-gray-900 font-bold`}>Edit Pot</DialogTitle>
